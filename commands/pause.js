...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player) return client.sendTime(message.channel, "❌ | **Não está jogando nada agora...**");
        ...
        player.pause(true);
        let embed = new MessageEmbed().setAuthor(`Paused!`, client.botconfig.IconURL).setColor("RANDOM").setDescription(`Type \`${GuildDB.prefix}resume\` ontinuar escutando!`);
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!player) return client.sendTime(interaction, "❌ | **Não está jogando nada agora...**");
            if (player.paused) return client.sendTime(interaction, "A música já está pausada!");
            player.pause(true);
